...
def load_als(index):
    ...
    for year in years:
        ...
        paths = [
            f"/vsicurl/https://zenodo.org/records/{data_id}/files/Predicted_height_from_ALS_Sentinel_model_{year}_batch_{no}.tif?download=1"
            for no in range(14)
        ]

        with TemporaryDirectory() as folder:
            text = f"{folder}/canopy.txt"
            ...
            check_call(
                f"gcloud storage cp {canopy} gs://gee-ramiqcom-s4g-bucket/opengeohub_summerschool_2026/als_canopy_height/{name}.tif",
                shell=True,
            )
